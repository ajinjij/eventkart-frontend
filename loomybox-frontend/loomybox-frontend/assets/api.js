...
// ---- Brand mark: a gift box with a ribbon, drawn in-house (no stock icons/photos) ----
// Used at small size in the header and large size on the homepage hero.
function brandMarkSvg() {
  return `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
    <rect x="8" y="26" width="48" height="32" rx="6" fill="var(--plum)"/>
    <rect x="27" y="14" width="10" height="44" fill="var(--pink)"/>
    <rect x="6" y="38" width="52" height="10" fill="var(--pink)"/>
    <path d="M32 14c-6-8-18-6-14 2c3 6 10 6 14 0" fill="none" stroke="var(--pink)" stroke-width="5" stroke-linecap="round"/>
    <path d="M32 14c6-8 18-6 14 2c-3 6-10 6-14 0" fill="none" stroke="var(--pink)" stroke-width="5" stroke-linecap="round"/>
    <rect x="27" y="10" width="10" height="8" rx="2" fill="var(--plum)"/>
  </svg>`;
}
...
async function renderNav(activePage) {
  ...
  nav.innerHTML = `
    <div class="logo-block">
      <a href="index.html" class="logo" id="site-logo"><span class="logo-mark">${brandMarkSvg()}</span>Loomy<span class="logo-accent">box</span></a>
    </div>
    ${vendorPillHtml}
    ${searchHtml}
    ${locationHtml}
  ...
  loadSiteSettings().then(settings => {
    ...
    if (logoEl && settings.siteName && settings.siteName !== "Loomybox") {
      logoEl.innerHTML = `<span class="logo-mark">${brandMarkSvg()}</span>${settings.siteName}`;
    }
  });
}
